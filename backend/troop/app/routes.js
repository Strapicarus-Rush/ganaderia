module.exports = function (Troop, router, Sequelize, dbConnection, Dictionary, authorizationCheck) {
    ...
    router.route('/register')
        ...
        .post(authorizationCheck, async (req, res) => { // POST
            try {
                ...
                const createdTroop = await dbConnection.transaction({ isolationLevel: Sequelize.Transaction.ISOLATION_LEVELS.READ_COMMITTED }, async (t) => {
                ...
                if (!createdTroop) return res.status(401).json({data:{0:0}, message: Dictionary.fail_registration('es'), hasErr: true });
                return res.status(200).json({ message: Dictionary.success_registration('es'), data: createdTroop });
            } catch (error) {
    ...
    router.route('/get/:id/:id_company')
        ...
        .get(authorizationCheck, async (req, res) => { // GET
            try {
                ...
                const troop = Troop.findOne({
                    where: { id_company: company, id: id }
                })
                return res.status(200).json({ message: '', hasErr: false, data: troop })
            } catch (err) {
                if (process.env.devmode) return res.status(200).json({ message: err.stack || err.message || err, hasErr: true })
        ...
        .put(authorizationCheck, async (req, res) => { // PUT
            try {
                ...
                const edited = await dbConnection.transaction({ isolationLevel: Sequelize.Transaction.ISOLATION_LEVELS.READ_COMMITTED }, async (t) => {
                    const troop = await Troop.update(
                        {
                            id_company: company,
                            name: name,
                            description: description,
                ...
                return res.status(200).json({ message: Dictionary.success_edit('es'), data: troop, hasErr: false });
            } catch (err) {
                if (process.env.devmode) return res.status(200).json({ message: err.stack || err.message || err, hasErr: true })
    ...
    router.route('/delete/:id/:id_company')
        ...
        .delete(async (req, res) => { // DELETE
            try {
                ...
                if (isNaN(company)) throw new Error(Dictionary.unknown_company('es'));
                if (isNaN(id)) throw new Error(Dictionary.unknown_identity('es'));
                const deletedTroop = await dbConnection.transaction({ isolationLevel: Sequelize.Transaction.ISOLATION_LEVELS.READ_COMMITTED }, async (t) => {
                    const troop = await Troop.update(
                        {
                            deleted: true
                        }, { where: { id_company: company, id: id }, transaction: t });
                ...
                if (!deletedTroop) return res.status(401).json({data:{0:0}, message: Dictionary.fail_deletion('es'), hasErr: true });
