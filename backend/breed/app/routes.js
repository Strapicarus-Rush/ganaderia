module.exports = function (Breed, router, Sequelize, dbConnection, Dictionary, authorizationCheck) {
    ...
    router.route('/get/:id/:id_company')
        ...
        .get(authorizationCheck, async (req, res) => { // GET
            try {
                ...
                const breed = Breed.findOne({
                    where: { id_company: company, id: id }
                })
                return res.status(200).json({ message: '', hasErr: false, data: breed })
            } catch (err) {
                if (process.env.devmode) return res.status(200).json({ message: err.stack || err.message || err, hasErr: true })
    ...
    router.route('/edit/:id/:id_company')
        ...
        .put(authorizationCheck, async (req, res) => { // PUT
            try {
                ...
                return res.status(200).json({ message: Dictionary.success_edit('es'), data: breed, hasErr: false });
            } catch (err) {
                if (process.env.devmode) return res.status(200).json({ message: err.stack || err.message || err, hasErr: true })
